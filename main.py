...
def check_missions_for_pumpkins():
    driver = login()
    driver.get("https://www.missionchief.com/")
    total_pumpkins_found = 0
    while True:
        try:
            ...
            mission_panels = mission_list.find_elements(By.CSS_SELECTOR, 'div.mission_panel')
            for mission in mission_panels:
                try:
                    pumpkin_link = mission.find_element(By.CSS_SELECTOR, 'a#easter-egg-link')
                    pumpkin_link.click()
                    total_pumpkins_found += 1
                except:
                    continue
            print(f"Total pumpkins found: {total_pumpkins_found}")
        except Exception as e:
            print("Error processing missions:", e)
        time.sleep(120)
...
def dispatch_all_missions(driver):
    ...
    for m_number, mission_info in shared_missions_data.items():
        ...
        if config.getboolean('missions', 'should_wait_before_missions', fallback=False):
            time.sleep(config.getint('missions', 'should_wait_before_missions_time', fallback=False))
...
if __name__ == "__main__":
    ...
    pumpkin_thread = Thread(target=check_missions_for_pumpkins)
    pumpkin_thread.start()
    ...
    while True:
        ...
        missions_per_thread = len(total_missions) // threads
        extra_missions = len(total_missions) % threads
        with ThreadPoolExecutor(max_workers=threads) as executor:
            futures = []
            start = 0
